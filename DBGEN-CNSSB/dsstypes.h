...
typedef struct
{
 ...
 //   DSS_HUGE	    *okey;  /*for clustering line items*/
 //   int             linenumber; /*integer, constrain to max of 7*/
 //   //long            custkey; //from customer_t
 //   //long            partkey; //from part_t
 //   //long            suppkey; //from supplier_t
	////char            orderdate[DATE_LEN]; //from date_t
 ...
 //   char            mktsegment[MAXAGG_LEN + 1];
	...
	////from supplier_t
 //   //long            suppkey;
 //   char            name_supplier[S_NAME_LEN + 1];
 //   char            address_supplier[S_ADDR_MAX + 1];
 //   int             alen_supplier; 
 //   char            city_supplier[CITY_FIX +1];
 //   int             nation_key_supplier;
 //   char            nation_name_supplier[S_NATION_NAME_LEN+1];
 //   int             region_key_supplier;
 //   char            region_name_supplier[S_REGION_NAME_LEN+1];
 //   char            phone_supplier[PHONE_LEN + 1];
	supplier_t sup;

	////from part_t
 //   //long           partkey;
 //   char           name_part[P_NAME_LEN + 1];
 //   int            nlen_part;
 //   char           mfgr[P_MFG_LEN + 1];
 //   char           category[P_CAT_LEN + 1];
 //   char           brand[P_BRND_LEN + 1];
...
long	mk_nlineorder	PROTO((long index, nlineorder_t * n, long upd_num, part_t *p, supplier_t *s, customer_t * c, date_t *d,  order_t *o));
