...
long
	mk_nlineorder(long index, nlineorder_t * n, long upd_num,  part_t *p, supplier_t *s, customer_t * c, date_t *d,  order_t *o)
	{
		n->part = *p;
		n->sup = *s;
		n->cust = *c;
...
char * month_names[]={"January","February","March","April", "May","June","July","Augest", "September","Octorber","November","December"};
...
/*specific function for nosql because index reaches max value for huge data number so we compute date on a base period*/
long
mk_ndate(long index,date_t *d, long base)
{
	mk_date(index%base,d);
	return (0);
}
