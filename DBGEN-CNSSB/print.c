...
int 
insertCustFields(FILE *fp, customer_t *c)
{
...
char * weekday_names_to_prt[]={"Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"};
char * month_names_to_prt[]={"January","February","March","April", "May","June","July","Augest", "September","Octorber","November","December"};
	...
	pr_nline (nlineorder_t * n, int mode)
{
   ...
   int             monthnuminyear;
   int             weeknuminyear;
   char			   commitDateExt[DATE_LEN_EXT];
	...
	if (fp_l == NULL || mode != last_mode)
	{
		if (fp_l) 
			fclose(fp_l);
		fp_l = print_prep(NLINE, mode);
		last_mode = mode;
	}

	for (i = 0; i < n->order.lines; i++)
	{
		PR_STRT(fp_l);
		PR_HUGE(fp_l, n->order.lineorders[i].okey);
		PR_INT(fp_l, n->order.lineorders[i].linenumber);

		PR_INT(fp_l, n->order.lineorders[i].custkey);//cust
		insertCustFields(fp_l, &n->cust);
		PR_INT(fp_l, n->order.lineorders[i].partkey);//part
		insertPartFields(fp_l, &n->part);
		PR_INT(fp_l, n->order.lineorders[i].suppkey);//sup
		insertSuppFields(fp_l, &n->sup);
		//PR_STR(fp_l, n->order.lineorders[i].orderdate, DATE_LEN);//date
		insertDateFields(fp_l, &n->date);

		//Move from the end to here and changed the format
		commitDate = D_STARTDATE + atoi(n->order.lineorders[i].commit_date);
		localTime = localtime(&commitDate);
		/*make Sunday be the first day of a week */
		daynuminweek=((long)localTime->tm_wday+1)%7+1;
		monthnuminyear=(long)localTime->tm_mon+1;
		strncpy(dayofweek, weekday_names_to_prt[daynuminweek-1], D_DAYWEEK_LEN+1);
		strncpy(month, month_names_to_prt[monthnuminyear-1],D_MONTH_LEN+1);
		year=(long)localTime->tm_year + 1900;
		daynuminmonth=(long)localTime->tm_mday;
		yearmonthnum=year * 100 + monthnuminyear;
		sprintf(commitDateExt,"%s %d, %d",month,daynuminmonth,year);

		PR_STR(fp_l, commitDateExt, DATE_LEN_EXT);

		PR_STR(fp_l, n->order.lineorders[i].opriority, O_OPRIO_LEN);
		PR_INT(fp_l, n->order.lineorders[i].ship_priority);
		PR_INT(fp_l, n->order.lineorders[i].quantity);
		PR_INT(fp_l, n->order.lineorders[i].extended_price);
		PR_INT(fp_l, n->order.lineorders[i].order_totalprice);
		PR_INT(fp_l, n->order.lineorders[i].discount);
		PR_INT(fp_l, n->order.lineorders[i].revenue);
		PR_INT(fp_l, n->order.lineorders[i].supp_cost);
		PR_INT(fp_l, n->order.lineorders[i].tax);
		PR_STR_LAST(fp_l, n->order.lineorders[i].shipmode, O_SHIP_MODE_LEN);
		PR_END(fp_l);
	}
	...
	pr_part(part_t *part, int mode)
{
	...
	insertPartFields(p_fp, part);
	PR_END(p_fp);
	return(0);
}

int 
insertPartFields(FILE *p_fp, part_t *part)
{
...
int 
insertSuppFields(FILE *fp, supplier_t *supp)
{
...
int 
insertDateFields(FILE *d_fp, date_t *d)
{
	...
	vrf_nline(order_t *o, int mode)
{
	...
	{
		VRF_STRT(NLINE);
		VRF_HUGE(NLINE, o->lineorders[i].okey);
		VRF_INT(NLINE, o->lineorders[i].linenumber);
		VRF_INT(NLINE, o->lineorders[i].custkey);
		VRF_INT(NLINE, o->lineorders[i].partkey);
		VRF_INT(NLINE, o->lineorders[i].suppkey);
		VRF_STR(NLINE, o->lineorders[i].orderdate);
		VRF_STR(NLINE, o->lineorders[i].opriority);
		VRF_INT(NLINE, o->lineorders[i].ship_priority);
		VRF_INT(NLINE, o->lineorders[i].quantity);
		VRF_INT(NLINE, o->lineorders[i].extended_price);
		VRF_INT(NLINE, o->lineorders[i].order_totalprice);
		VRF_INT(NLINE, o->lineorders[i].discount);
		VRF_INT(NLINE, o->lineorders[i].revenue);
		VRF_INT(NLINE, o->lineorders[i].supp_cost);
		VRF_INT(NLINE, o->lineorders[i].tax);
		VRF_STR(NLINE, o->lineorders[i].commit_date);
		VRF_STR(NLINE, o->lineorders[i].shipmode);
		VRF_END(NLINE);
	}
